...
UPSERT_GOLD_CSGOSTATS_DAILY = """
INSERT INTO gold.csgostats_daily (date, entity_id, avg_value, min_value, max_value, records_count)
SELECT
...
def gold_aggregate_csgostats_daily():
    pg_hook = PostgresHook(postgres_conn_id="postgres_datawarehouse")
    pg_hook.run(UPSERT_GOLD_CSGOSTATS_DAILY)
    logger.info("Gold csgostats_daily aggregation completed.")