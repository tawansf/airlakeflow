...
UPSERT_GOLD_CRYPTO_DAILY = """
INSERT INTO gold.crypto_daily (date, entity_id, avg_value, min_value, max_value, records_count)
SELECT
...
def gold_aggregate_crypto_daily():
    pg_hook = PostgresHook(postgres_conn_id="postgres_datawarehouse")
    pg_hook.run(UPSERT_GOLD_CRYPTO_DAILY)
    logger.info("Gold crypto_daily aggregation completed.")