...
_PROJECT_ROOT = _DAGS_DIR.parent
_MIGRATIONS_DIR = _DAGS_DIR / "sql" / "migrations"
_INIT_SCHEMAS_FILE = _PROJECT_ROOT / "scripts" / "002_create_schemas.sql"
...
# Read SQL file content so PostgresOperator gets SQL, not a path (Airflow would treat path as Jinja template).
INIT_SCHEMAS_SQL = _INIT_SCHEMAS_FILE.read_text(encoding="utf-8") if _INIT_SCHEMAS_FILE.exists() else "CREATE SCHEMA IF NOT EXISTS bronze; CREATE SCHEMA IF NOT EXISTS silver; CREATE SCHEMA IF NOT EXISTS gold;"
