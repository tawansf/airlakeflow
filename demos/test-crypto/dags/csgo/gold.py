...
UPSERT_GOLD_CSGO_DAILY = """
INSERT INTO gold.csgo_daily (date, entity_id, avg_value, min_value, max_value, records_count)
SELECT
...
def gold_aggregate_csgo_daily():
    pg_hook = PostgresHook(postgres_conn_id="postgres_datawarehouse")
    pg_hook.run(UPSERT_GOLD_CSGO_DAILY)
    logger.info("Gold csgo_daily aggregation completed.")