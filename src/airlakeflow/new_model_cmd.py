...
def run_new_model(
    ...
    partition_by_kw = f', partition_by="{partition_by}"' if partition_by else ""
    partition_column = (
        f"{partition_by} = Field.date()  # partition key (required for PARTITION BY RANGE)\n    "
        if partition_by
        else ""
    )
    ...
    content = _TPL_MODEL.format(
        ...
        table=table,
        layer=layer_name,
        partition_by_kw=partition_by_kw,
