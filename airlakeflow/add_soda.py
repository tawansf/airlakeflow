...
def _ensure_etl_contracts(project_root: Path, etl_name: str, env: Environment) -> tuple[bool, bool]:
    ...
    if not bronze_path.exists():
        bronze_path.write_text(_render_contract(env, "contract_bronze.yaml.j2", etl_name), encoding="utf-8")
        created_bronze = True
    if not silver_path.exists():
        silver_path.write_text(_render_contract(env, "contract_silver.yaml.j2", etl_name), encoding="utf-8")
        created_silver = True
...
def _inject_soda_into_pipeline(project_root: Path, etl_name: str) -> bool:
    ...
    if "run_soda_scan_and_persist" in content or "soda_scan_bronze" in content or "quality_bronze" in content:
        return False
...
"""
    ...
    old_chain = re.compile(
        r"(\s+)ingestion >> transformation >> gold_aggregate\s*$", re.MULTILINE
    )
    new_chain = r"\1ingestion >> quality_bronze >> transformation >> quality_silver >> gold_aggregate"
