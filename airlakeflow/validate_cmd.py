...
def _compose_services_up(project_root: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        if not out or "name" not in out:
            return False, "Stack not running (run: docker compose up -d)"
        if "exited" in out:
            return False, "Some containers exited (run: docker compose up -d)"
        return False, "Stack not running (run: docker compose up -d)"
    except FileNotFoundError:
        return False, "docker compose not found"
    except subprocess.TimeoutExpired:
...
def run_validate(
    ...
    if verbose:
        print(f"Validating project: {root}\n")

    if check_structure:
        ok, missing = _structure_ok(root)
        if verbose:
            if ok:
                print("  [OK] Project structure (dags/, soda/, scripts/, docker-compose.yaml)")
            else:
                print("  [FAIL] Project structure missing:", ", ".join(missing))
        if not ok:
            all_ok = False
        ...
        if verbose:
            if ok2:
                print("  [OK] Key files (setup_database.py, soda/configuration.yaml)")
            else:
                print("  [FAIL] Missing key files:", ", ".join(missing2))
        if not ok2:
            all_ok = False
    ...
    if check_docker:
        ...
        if not ok:
            ...
            if verbose:
                if ok2:
                    print("  [OK] Docker Compose:", msg2)
                else:
                    print("  [FAIL] Docker Compose:", msg2)
            if not ok2:
                all_ok = False
    ...
    if verbose:
        ...
        if all_ok:
            ...
            print("Validation failed. Fix the items above and run 'alf validate' again.")
    return all_ok
