...
def run_new_etl(
    ...
    print(f"ETL '{name}' created in dags/{name_snake}/")
    print(f"  - pipeline.py, bronze.py, silver.py" + (", gold.py" if with_gold else ""))
    print(f"  - transformations/{entity_snake}.py")
    if with_contracts:
        print(f"  - soda/contracts/{entity_snake}_bronze.yaml, {entity_snake}_silver.yaml")
    print("  Create the migrations later with: alf new migration <name> --dag " + name_snake + " --layer bronze|silver|gold")
