...
def run_new_migration(
    ...
    content = tpl.replace("{{ name }}", name).replace("{{ dag }}", dag).replace("{{ table }}", table_snake)

    filepath = migrations_dir / filename
    filepath.write_text(content.strip(), encoding="utf-8")

    print(f"Migration created: dags/sql/migrations/{filename}")
    print(f"  DAG: {dag} | Layer: {layer}")
    print("  Edit the file to adjust columns and indices.")
